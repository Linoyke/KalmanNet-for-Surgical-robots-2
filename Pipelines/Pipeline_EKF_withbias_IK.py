...
class Pipeline_EKF:
    ...
    def NNTrain(self, SysModel, cv_input, cv_target, train_input, train_target,cv_des,train_des, path_results, MaskOnState=False, randomInit=False, cv_init=None, train_init=None, train_lengthMask=None, cv_lengthMask=None):
        ...
        for ti in range(0, self.N_steps):
            ...
            train_des_batch = torch.zeros([self.N_B, SysModel.n, SysModel.T]).to(self.device)
            ...
            if randomInit:
                ...
                self.model.InitSequence(SysModel.m1x_0[n_e, :].unsqueeze(-1), SysModel.T)
            
            if self.args.CompositionLoss2:
                y_hat = torch.zeros([self.N_B, SysModel.n, SysModel.T]).to(self.device)
                bias_prior = torch.zeros([self.N_B, SysModel.n, SysModel.T]).to(self.device)

                for t in range(0, SysModel.T):
                    x_out_training_batch_t, y_out_training_batch_t, bias_prior_t = self.model(y_training_batch[:, :, t],train_des_batch[:,:,t])
                    x_out_training_batch[:, :, t] = x_out_training_batch_t.squeeze(-1)
                    y_hat[:, :, t] = y_out_training_batch_t.to(self.device)
                    bias_prior[:, :, t] =bias_prior_t.to(self.device)
            else:
                for t in range(0, SysModel.T):
                    # x_out_training_batch_t, y_out_training_batch = self.model(y_training_batch[:, :, t],train_des_batch[:,:,t]).squeeze(-1)
                    x_out_training_batch[:, :, t] = self.model(y_training_batch[:, :, t],train_des_batch[:,:,t]).squeeze(-1)
            ...
            if self.args.CompositionLoss:
                ...
                if MaskOnState:
                    ...
                    if self.args.randomLength:
                        ...
                        if self.args.CompositionLoss2:
                            # MSE_trainbatch_linear_LOSS = self.loss_fn(x_out_training_batch[:,:3,:], train_target_batch[:,:3,:]) + self.alpha*self.loss_fn(y_hat,y_training_batch) #(1-self.alpha)
                            MSE_trainbatch_linear_LOSS = self.loss_fn(x_out_training_batch[:,:3,:], train_target_batch[:,:3,:]) + self.alpha*self.loss_fn(y_hat - bias_prior,y_training_batch) #(1-self.alpha)
                        else:
                            MSE_trainbatch_linear_LOSS = self.loss_fn(x_out_training_batch[:,:3,:], train_target_batch[:,:3,:])
            ...
            with torch.no_grad():
                ...
                if randomInit:
                    ...
                    self.model.InitSequence(cv_target[:,:,0].unsqueeze(-1), SysModel.T_test)
                
                if self.args.CompositionLoss2:
                    for t in range(0, SysModel.T_test):
                        
                        x_out_cv_batch_t, y_out_cv_batch_t, bias_prior_t_cv = self.model(cv_input[:, :, t],cv_des[:,:,t])
                        x_out_cv_batch[:, :, t] = x_out_cv_batch_t.squeeze(-1)
                else:
                    for t in range(0, SysModel.T_test):
                        x_out_cv_batch[:, :, t] = self.model(cv_input[:, :, t],cv_des[:,:,t]).squeeze(-1)
                ...
                if MaskOnState:
                    ...
                    if self.args.randomLength:
                        ...
                        MSE_cvbatch_linear_LOSS = self.loss_fn(x_out_cv_batch[:,:3,:], cv_target[:,:3,:])
        ...
        plot_training_curves(self.MSE_train_dB_epoch, self.MSE_cv_dB_epoch, save_path="loss_curve_KNET.png")
        return [self.MSE_cv_linear_epoch, self.MSE_cv_dB_epoch, self.MSE_train_linear_epoch, self.MSE_train_dB_epoch]

    def NNTest(self, SysModel, test_input, test_target, test_des, path_results, MaskOnState=False, randomInit=False, test_init=None, load_model=False, load_model_path=None, test_lengthMask=None):
        # Load model
        if load_model:
        ...
        # test_input_mean = test_input.mean(dim=(0, 2), keepdim=True)
        # test_input_std = test_input.std(dim=(0, 2), keepdim=True)
        # test_input = (test_input - test_input_mean) / test_input_std
        if self.args.CompositionLoss2:
            for t in range(0, SysModel.T_test):
                x_out_t, y_out_t, bias_out_t = self.model(test_input[:, :, t],test_des[:,:,t])
                x_out_test[:, :, t] = x_out_t
        else:
            for t in range(0, SysModel.T_test):
                x_out_test[:, :, t] = self.model(test_input[:, :, t],test_des[:,:,t]).squeeze(-1)
        ...
        # Print Run Time
        print("Inference Time KalmanNet:", t)
