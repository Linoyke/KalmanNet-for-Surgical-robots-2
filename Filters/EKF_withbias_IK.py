...
from Utils import project_to_box
from Simulations.raven_ii_matlab.parameters_withbias_trainIK import x_limits, y_limits, z_limits
...
class ExtendedKalmanFilterWithIK(nn.Module):
    ...
    # Ensure m1x_posterior and m1x_prior have the correct shape: [batch_size, m, 1]
        self.m1x_prior_pos = self.f(self.m1x_posterior[:, :3, :],des)
        self.bias_prior = self.m1x_posterior[:, 3:,:]
        self.m1x_prior = torch.cat((self.m1x_prior_pos,self.bias_prior),dim =1) # Shape should remain [150, 6] or [150, 6, 1]
        

    # Compute Jacobians (Ensure these operations do not introduce extra dimensions)
        #self.g = lambda x: self.f(self.m1x_posterior[:, :3, :], des)
        F_k = torch.eye(3, device=self.m1x_posterior.device).unsqueeze(0).repeat(self.m1x_posterior.shape[0], 1, 1)
        self.UpdateJacobians(F_k, getJacobian(self.m1x_prior[:,:3,:],des_batch=None, f_func = self.h_nn))
        #self.UpdateJacobians(F_k, getJacobian(self.m1x_prior,des_batch=None, f_func = self.h_nn))
        # self.UpdateJacobians(getJacobian(self.m1x_posterior[:, :3, :],des, self.f), getJacobian(self.m1x_prior[:,:3,:],des_batch=None, f_func = self.h_nn))
        # F_k = torch.eye(3, device=self.m1x_posterior.device).unsqueeze(0).repeat(batch_size, 1, 1)
        #self.UpdateJacobians(getJacobian(self.m1x_posterior[:, :3, :], self.g), getJacobian(self.m1x_prior[:,:3,:], self.h_nn))
    ...
    # Predict the first moment of y (position)
        ...
        self.j_t = self.h_nn(position)  # Pass through IK network, shape [batch_size, 3]
    
    # Add bias to the prediction
        self.m1y = self.j_t.unsqueeze(-1) + self.m1x_prior[:, 3:]  # Add bias to prediction, shape [batch_size, 3]
    ...
    def Correct(self):
        ...
        self.m1x_posterior = project_to_box(self.m1x_posterior,x_limits, y_limits, z_limits)
    ...
    def UpdateJacobians(self, F, H):
        ...
        F.to(self.device)
        F_pos = F
        B = F_pos.shape[0] # get the batch size
        device = F_pos.device
        
        # Create identity [3, 3], and expand it into [B, 3, 3]
        F_bias = torch.eye(3, device=device).expand(B, 3, 3)
        
        # Allocate the full Jacobian [B, 6, 6]
        F_full = torch.zeros(B, 6, 6, device=device)
        H_full = torch.zeros(B, 3, 6, device=device) ######### added
        
        # Fill in the top-left block with F_pos
        F_full[:, :3, :3] = F_pos
        H_full[:, :, :3] = H ######## added
        
        # Fill in the bottom-right block with identity (for unchanged bias)
        F_full[:, 3:, 3:] = F_bias 
        H_full[:, :, 3:] = F_bias ######## added
        self.batched_F = F_full.to(self.device)
        self.batched_F_T = torch.transpose(F_full, 1, 2)
        # self.batched_H = H.to(self.device)
        self.batched_H = H_full.to(self.device)
        # self.batched_H_T = torch.transpose(H, 1, 2)
        self.batched_H_T = torch.transpose(H_full, 1, 2)
    ...
    def GenerateBatch(self, y,des):
        ...
        self.y_hat = torch.zeros(self.batch_size, self.n, T).to(self.device)
        self.bias_prior_traj = torch.zeros(self.batch_size, self.n, T).to(self.device)
        self.j_t_traj = torch.zeros(self.batch_size, self.n, T).to(self.device)
        ...
        for t in range(T):
            ...
            des_t = torch.unsqueeze(des[:, :, t], 2)
            xt, sigmat = self.Update(yt,des_t)
            self.x[:, :, t] = torch.squeeze(xt, 2)
            self.sigma[:, :, :, t] = sigmat
            if self.args.CompositionLoss2:
                self.y_hat[:, :, t] = torch.squeeze(self.m1y, 2)
                self.bias_prior_traj[:, :, t] = torch.squeeze(self.bias_prior, 2)
                self.j_t_traj[:, :, t] = torch.squeeze(self.j_t.unsqueeze(-1), 2)
