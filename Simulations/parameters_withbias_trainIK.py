...
#### projection box limits in mm #####
x_limits = (-90, 10) #-70,0
y_limits = (-70, 70) #-70, 70
z_limits = (-110, 20) #-100,20
...
def f(x,des, jacobian=False):
    jac = torch.eye(m).repeat(x.shape[0], 1, 1).to(x.device)
    if jacobian:
        return x, jac
    return x

def f_with_action(x,des,jacobian=False):
    jac = torch.eye(m).repeat(x.shape[0], 1, 1).to(x.device)
    if jacobian:
        return x+des, jac
    return x+des
...
# def getJacobian(x_batch, des_batch, f_func):
#     """
#     Compute per-sample Jacobians ∂f/∂x for a batch.

#     Args:
#         x_batch: Tensor of shape [B, m, 1]
#         des_batch: Tensor of shape [B, m, 1] or None
#         f_func: function that takes either f(x) or f(x, des)

#     Returns:
#         jacobian_batch: Tensor of shape [B, m_out, m]
#     """
#     B, m, _ = x_batch.shape
#     x_flat = x_batch.squeeze(-1)  # [B, m]
#     jacobian_list = []

#     for i in range(B):
#         x_i = x_flat[i].clone().detach().requires_grad_(True)  # [m]

#         # Choose function form based on whether des_batch is used
#         if des_batch is not None:
#             des_i = des_batch[i].squeeze(-1).detach()  # [m]
#             def f_wrapped(xi):
#                 return f_func(xi.view(1, -1, 1), des_i.view(1, -1, 1)).squeeze(0)
#         else:
#             def f_wrapped(xi):
#                 return f_func(xi.view(1, -1)).squeeze(0)  # assume h_nn expects [1, m]

#         Ji = autograd.functional.jacobian(f_wrapped, x_i)  # shape: [m_out, m]
#         jacobian_list.append(Ji)

#     jacobian_batch = torch.stack(jacobian_list, dim=0)  # [B, m_out, m]
...
#     for i in range(B):
#         x_i = x_flat[i].clone().detach().requires_grad_(True)  # shape: [m]

#         # Define a function that takes unbatched x_i and returns unbatched output
#         if des_batch is not None:
#             des_i = des_batch[i].squeeze(-1).detach()  # shape: [m]

#             def f_wrapped(xi):
#                 out = f_func(xi.view(1, -1, 1), des_i.view(1, -1, 1))  # [1, m_out, 1]
#                 return out.squeeze(0).squeeze(-1)  # [m_out]
#         else:
#             def f_wrapped(xi):
#                 out = f_func(xi.view(1, -1))  # [1, m_out]
#                 return out.squeeze(0)        # [m_out]

#         Ji = autograd.functional.jacobian(f_wrapped, x_i)  # shape: [m_out, m]

#         # Fix in case autograd returned shape [1, m_out, m] or [m_out, m, 1]
#         if Ji.dim() == 3:
#             Ji = Ji.squeeze(0) if Ji.shape[0] == 1 else Ji.squeeze(-1)
...
def getJacobian(x_batch, des_batch, f_func):
    ...
    des_batch = des_batch.to(device) if des_batch is not None else None
    ...
    for i in range(B):
        x_i = x_flat[i].clone().detach().to(device).requires_grad_(True)  # [m]

        # Define a function that takes unbatched x_i and returns unbatched output
        if des_batch is not None:
            des_i = des_batch[i].squeeze(-1).detach().to(device)  # [m]
        ...
        Ji = autograd.functional.jacobian(f_wrapped, x_i)  # [m_out, m]
